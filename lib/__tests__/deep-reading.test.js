...
describe('Deep Reading Utilities', () => {
  ...
  it('should ensure proper conclusion for English', () => {
    ...
    assert(
      result.includes('Overall conclusion:'),
      'Should have English conclusion prefix'
    );
  });
  ...
  it('should preserve existing conclusion', () => {
    ...
    const result = ensureFinalConclusion('en', text);
    assert(
      result.endsWith('Overall conclusion: This is already a conclusion.'),
      'Should preserve existing conclusion'
    );
  });

  it('should format complete deep reading for CJK', () => {
    const longText =
      '這是一個很長的分析文本。' + '這包含很多細節和解釋。'.repeat(20);
    ...
    assert(
      measurement.chars <= 400,
      'Should not exceed maximum character count'
    );
  ...
  it('should format complete deep reading for English', () => {
    const longText =
      'This is a very long analysis text. '.repeat(50) +
      'This contains many details and explanations.';
  ...
  it('should handle short text by preserving it', () => {
    ...
    assert(
      result.includes('Overall conclusion:'),
      'Should still add conclusion to short text'
    );
  });
  ...
  it('should compress content while preserving conclusion', () => {
    ...
    const longText =
      'This is really very quite rather extremely long content with many unnecessary filler words. ' +
    ...
    assert(
      compressed.includes('conclusion paragraph'),
      'Should preserve conclusion content'
    );
  });
});
