...
1. 建立 `.env.app.local` 並填入 AdMob 測試 ID。 _Create `.env.app.local` with Google test AdMob IDs._
...
| `NEXT_PUBLIC_ADMOB_BANNER_ID` | AdMob Banner 測試/正式 ID | Banner ad unit (Google test ID by default) |
| `NEXT_PUBLIC_ADMOB_INTERSTITIAL_ID` | AdMob Interstitial 測試/正式 ID | Interstitial ad unit |
| `NEXT_PUBLIC_ADMOB_REWARDED_ID` | AdMob Rewarded 測試/正式 ID | Rewarded ad unit |
